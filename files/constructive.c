...
void set_nurse_time(struct INSTANCE* ip, int nursei){
    ...
    for(int j = 0;
        j < ip->nJobs;
        ++j){ // Reset all times for nursei for all jobs to -1. Recall that timeMatrix stores time at which each nurse i (row) does each job j (column). If i doesn't do job j, then =-1.
