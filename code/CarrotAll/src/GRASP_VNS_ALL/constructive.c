...
void SolnToPythomFormat(struct INSTANCE* ip, int* solMatrixPointer, double* timeMatrixPointer, double* nurseWaitingTimePointer, double* nurseTravelTimePointer, double* violatedTWPointer,
    ...
    //printf("Enter SolnToPythonFormat\n");
    ...
    //printf("Exit SolnToPythonFormat \n");
...
int CheckSkillsDSFirst(struct INSTANCE* ip, int job, int nursei){
    ...
    //printf("Enter CheckSillsDSFirst\n");
    ...
    //printf("Exit CheckSkillsDSFirst \n");
...
void InitialJobAssignment(struct INSTANCE* ip){
    ...
    //printf("Enter InitialJobAssignment\n");
...
int ClosestUnallocatedPointNurse(struct INSTANCE* ip, int* allocatedJobs, int job, int nurse){
    ...
    //printf("Exit ClosestUnallocatedPointNurse\n");
...
int FurthestUnallocatedPointNurse(struct INSTANCE* ip, int* allocatedJobs, int nurse){
    ...
    //printf("Enter FurthestUnallocatedPointNurse\n");
    ...
    // exit(0);
    //printf("Exit FurthestUnallocatedPointNurse\n");
...
void PrintIntMatrix(int** matrix, int nRows, int nCols){
    ...
    //printf("Exit PrintIntMatrix\n");

}

void PrintIntMatrixOne(int* matrix, int nRows, int nCols){

    //printf("Enter PrintIntMatrixOne\n");
    ...
    //printf("Exit PrintIntMatrixOne\n");

}

void PrintDoubleMatrixOne(double* matrix, int nRows, int nCols){

    //printf("Enter PrintDoubleMatrixOne\n");
    ...
    //printf("Exit PrintDoubleMatrixOne\n");
...
double TravelTimeFromDepot(struct INSTANCE* ip, int nurse, int job){
    ...
    //printf("Enter TravelTimeFromDepot\n");
    //printf("Exit TravelTimeFromDepot\n");
...
double TravelTimeToDepot(struct INSTANCE* ip, int nurse, int job){
    ...
    //printf("Enter TravelTimeToDepot\n");
    //printf("Exit TravelTimeFromDepot\n");
...
int ReportSolution(struct INSTANCE* ip){
    ...
    //printf("Enter ReportSolution\n");
    //printf("ReportSolution\n");
    SolutionQuality(ip, 1);

    //printf("Exit ReportSolution\n");
        ...
        if(InsertJobAtPosition(ip, job, ni, j) < 0){ // If job cannot be inserted into nurse ni's route in position j, exit for loop.
            ...
            if(propQuality > bestQuality){ // If this solution has better quality than current best quality solution
                ...
                if(firstSwitch > 0){ // Is this needed?
                    //printf("Exit BestJobInsertion firstSwitch>0\n");
                    return 0;
                }
            }
            RemoveJob(ip, job, ni); //Reverse job insertion, go back to original solution.
...
int InsertJobAtPosition(struct INSTANCE* ip, int job, int ni, int posi){
    ...
    //printf("Enter InsertJobAtPosition\n");
    ...
    if(ip->solMatrix[ni][job] > -1){ //The job is already assigned to that nurse.
        ...
        //printf("Exit InsertJobAtPosition Job already assigned to that nurse\n");
        return -1;
    }
    ...
    if(CheckSkills(ip, job, ni) < 1 && ip->doubleService[job] < 0.5){ //why 0.5? doubleService is an int array.
        ...
        //printf("Exit InsertJobAtPosition unskilled\n");
        return -1;
    }
    ...
    if((retVal==0) || (avJobs==posi)){
        ...
        SetTimesFrom(ip, ni);
        //printf("Exit InsertJobAtPosition retVal ==0\n");
        return 0;
    }
    SetNurseRoute(ip, ni); // Update allNurseRoutes for nurse ni.

    //printf("Exit InsertJobAtPosition\n");
...
int BestSyncDoubleSwitch(struct INSTANCE* ip){
    ...
    //printf("Enter BestSyncDoubleSwitch\n");
	...
	*/
    ...
    for(int i = 0; i < ip->nJobs; ++i){ //For each job i=0,...,nJobs
        ...
        for(int tn = 0; tn < ip->nNurses; ++tn){ //For each nurse tn=0,...,nNurses
            ...
            if(ip->doubleService[jobA]){ //If jobA is a double service
                if(CheckSkillsDSFirst(ip, jobA, tn) < 1){ //If no other nurse can do jobA with nurse tn. (capabilityOfDoubleService, if there's another nurse that can do the job with tn then =1, else = 0)
                    if(DEBUG_PRINT > 0){
    ...
    if(overAllBestFitness > baseQuality + TOL){
        ...
        if((overall_bestA < 0) || (overall_bestANurse < 0) || (overall_bestAPos < 0) || (overall_bestB < 0) || (overall_bestBNurse < 0) || (overall_bestBPos < 0)){
            ...
            //printf("Exit BestSyncDoubleSwitch overall_bestA < 0\n");
            return -1;
        }
        ...
        //printf("Exit BestSyncDoubleSwitch overAllBestFitness > baseQuality + TOL\n");
        return 1;
    }
    else{
        if(DEBUG_PRINT > 0){
        ...
        //printf("Exit BestSyncDoubleSwitch else\n");
        return -1;
    }

    //printf("Exit BestSyncDoubleSwitch\n");

} // END OF BestSyncDoubleSwitch function.

int RouteTwoExchange(struct INSTANCE* ip, int firstImprovement){

    //printf("Enter RouteTwoExchange\n");
        ...
        for(int job_from_ni = 0; job_from_ni < ip->nJobs; ++job_from_ni){ //For each job_from_ni 0,...,nJobs
            ...
            for(int nj = ni + 1; nj < ip->nNurses; ++nj){ //For each nurse nj = ni+1,..., nNurses
                ...
                if(ip->doubleService[job_from_ni]){ //If job_from_ni is a double service
                    if(CheckSkillsDS(ip, job_from_ni, secondNurseI, nj) < 1){ // returns ip->capabilityOfDoubleServices[secondNurseI][nj][jobdsindex], <1 means nj is not skilled to do the job with secondNurseI
                        continue;
                    }
                }
...
int ExchangeJobsInRoute(struct INSTANCE* ip, int ni, int job_from_ni, int nj, int job_from_nj){
    ...
    //printf("Enter ExchangeJobsInRoute\n");
    ...
    //printf("Exit ExchangeJobsInRoute\n");
...
int BestSwitch(struct INSTANCE* ip, int onlyInfeasible, double MAX_TIME){
    ...
    if(besti > -1){
        ...
        //printf("Exit BestSwitch besti > -1 return 0\n");
        return 0;
    }
...
void FreeMatrixInt(int** matrix, int nRows){
    ...
    //printf("Enter FreeMatrixInt\n");
    for(int i = 0; i < nRows; ++i){
        free(matrix[i]);
    }
    free(matrix);
    //printf("Exit FreeMatrixInt\n");
}

int CopyIntMatrix(int** source, int** destination, int nRows, int nCols){

    //printf("Enter CopyIntMatrix\n");
...
int TwoOptMove(struct INSTANCE* ip, int ni, int pos1, int pos2){
    ...
    if(pos1==pos2){
        ...
        //printf("Exit TwoOptMove pos1==pos2\n");
        return -1;
    }
    else if(pos1 > pos2){
        int aux = pos1;
    ...
    if(pos2 >= ip->nJobs){
        ...
        //printf("Exit TwoOptMove pos2 >= ip->nJobs\n");
        return -1;
    }
    ...
    if(nurseRoute[pos2] < 0){ // There's no job in position 2, so clearly not enough jobs in the route for this move
        free(nurseRoute);
        //printf("Exit TwoOptMove nurseRoute[pos2] < 0\n");
        return -1;
    }
    ...
    //printf("Exit TwoOptMove return 0\n");
...
int FindSecondNurseDS(struct INSTANCE* ip, int job, int currentNurse){
    ...
    //printf("Enter FindSecondNurseDS\n");
...
int SwitchNurse(struct INSTANCE* ip, int ni, int nj, int pi){
    ...
    if(ip->solMatrix[ni][pi] < 0){ //Job pi is not assigned to nurse ni
        //printf("Exit SwitchNurse solmatirx[ni][pi] < 0\n");
    ...
    if(ip->nurseSkilled[nj][pi] < 1 && ip->doubleService[pi] < 1){ //Nurse ni is not skilled to do job pi, and job pi is not a double service
        //printf("Exit SwitchNurse ni not skilled to do job pi\n");
        return -1; // Nothing to switch
    }
    ...
    if(ip->doubleService[pi] > 0){ // If job pi is a double service
        ...
        if(CheckSkillsDS(ip, pi, secondNurse, nj) < 1){ //returns ip->capabilityOfDoubleServices[secondNurse][nj][jobdsindex];
            //printf("Exit SwitchNurse check skills ds < 1\n");
            return -1;
        }
...
void SetNurseTime(struct INSTANCE* ip, int nursej){
    ...
    ip->nurseWaitingTime[nursej] = 0; // Reset waiting time for nursei
    ip->nurseTravelTime[nursej] = 0; // Reset travel time for nursei
    ...
    for(int j = 0; j < ip->nJobs; ++j){
        ip->nurseWaitingMatrix[nursej][j] = 0;
    }
    ...
    for(int j = 0; j < ip->nJobs; ++j){
        ip->nurseTravelMatrix[nursej][j] = 0;
    }

    // Determine the number of unavailable 'shifts' for nursei
    int numUnavail = ip->nurseUnavail[nursej];

    // Main for loop:
    for(int j = 0; j < ip->nJobs; ++j){ // Main for loop of function, going through all POSITIONS j=0,...,nJobs.
        if(ip->allNurseRoutes[nursej][j] < 0){ // If there is no job in position j of nursei's route, then we have reached the final position of nursei, break out of for loop.
            break;
        }
        ...
        if(prevPoint < -0.5){ // 'job' is the first job in nursei's route - there is no previous job, so prevPoint = -1.
            tTime = TravelTimeFromDepot(ip, nursej, job); // get_travel_time_from_depot = ip->nurseTravelFromDepot[nursei][job]
            if(ip->excludeNurseTravel){ // Exclude tTime (from nursei's home to this first job) when updating currentTime, but still include tTime when updating ip->nurseTravelTime[nursei]
                if(ip->jobTimeInfo[job][0] > ip->nurseWorkingTimes[nursej][0] - 0.001){ // If the start TW of 'job' is LATER than the start time of nursei's shift for the day
        ...
        ip->nurseTravelTime[nursej] += tTime; // Update the total travel time for nursei.
        ...
        ip->timeMatrix[nursej][job] = currentTime; // Update timeMatrix for nursei and job to be the current time that the nurse is at the location of the job.
        // End part one
        ...
        if(ip->dependsOn[job] > -1){
            int otherJob = ip->dependsOn[job];
            for(int prevCarerInd = 0; prevCarerInd < ip->nNurses; ++prevCarerInd){
                int prevCarer = ip->nurseOrder[prevCarerInd];
                if(prevCarer==nursej){
                    considerDependency = -1;
                    break;
                }
                if(ip->timeMatrix[prevCarer][otherJob] > 0){
                    if(aitOnly > 0){
                        ...
                        double laa = ip->timeMatrix[prevCarer][otherJob] - ip->mkMaxD[job];
                        if((laa >= startTW) && (arriveAt <= laa)){ //NB: arrive at is used here!
                            ip->mkMinD[job] = -1*ip->mkMinD[job];
                            ip->mkMaxD[job] = -1*ip->mkMaxD[job];
                        }
                    }
                    startTWMK = ip->timeMatrix[prevCarer][otherJob] + ip->mkMinD[job];
                    endTWMK = ip->timeMatrix[prevCarer][otherJob] + ip->mkMaxD[job];
                    considerDependency = 1;
                    break;
                }
            }
        ...
        if(arriveAt < worstStart){ // arriveAt is EARLIER than the latest start time
            ...
            ip->nurseWaitingMatrix[nursej][job] = waitingTime;
            ip->timeMatrix[nursej][job] += waitingTime; // Update timeMatrix for nursei and 'job' so that the time nursei actually starts 'job' isn't just the arrival time, it's after the waiting time.
        ...
        if(numUnavail > 0){
            for(int i = 0; i < numUnavail; ++i){
                ...
                if(arriveAt < ip->unavailMatrix[i][1][nursej] && arriveAt + ip->jobTimeInfo[job][2] > ip->unavailMatrix[i][1][nursej]){
                    waitingTime = ip->unavailMatrix[i][1][nursej] - arriveAt; //waiting time = time from currentTime to the start of the unavailable shift
                    //ip->nurseWaitingTime[nursei] += waitingTime;
                    ip->nurseWaitingMatrix[nursej][job] += waitingTime;
                    arriveAt = ip->unavailMatrix[i][2][nursej]; // update currentTime to be the end of the unavailable shift.
                    ip->timeMatrix[nursej][job] = arriveAt;
                ...
                else if(arriveAt >= ip->unavailMatrix[i][1][nursej] && arriveAt < ip->unavailMatrix[i][2][nursej]){
                    if(first == 0 && currentTime < ip->unavailMatrix[i][1][nursej]){
                        waitingTime = ip->unavailMatrix[i][1][nursej] - currentTime;
                        ip->nurseWaitingMatrix[nursej][job] = waitingTime;
                        first = 1;
                    }
                    arriveAt = ip->unavailMatrix[i][2][nursej]; // update currentTime to be the end of the unavailable shift.
    ...
    // Return to depot:
    if(prevPoint > -1){ // If a previous job is set, calculate the time it takes for the nurse to go from the last job (prevPoint) back to the depot (their home).
        double tTime2 = TravelTimeToDepot(ip, nursej, prevPoint); //get_travel_time_to_depot function returns ip->nurseTravelToDepot[nursei][prevPoint].
        ...
        ip->nurseTravelTime[nursej] += tTime2; //get_travel_time_to_depot function returns ip->nurseTravelToDepot[nursei][prevPoint].
    ...
    for(int j = 0; j < ip->nJobs; ++j){
        ip->nurseWaitingTime[nursej] += ip->nurseWaitingMatrix[nursej][j];
    }
...
void SetTimesFull(struct INSTANCE* ip){
    ...
    for(int j = 0; j < ip->nNurses; ++j){
        ...
        /*if(ip->qualityMeasure == 0){ // NB FOR AIT H ONLY 11/11/2021
            SetNurseTimeAitH(ip, nurse);
        }
        else{
...
void SetTimesFrom(struct INSTANCE* ip, int firstNurse){
    ...
    for(int i = 0; i < ip->nNurses; ++i){ //For each nurse i = 0,...,nNurses
        ...
        /*if(ip->qualityMeasure == 0){ // NB FOR AIT H ONLY 11/11/2021
            ...
            SetNurseTime(ip, nurse); //Note that set_nurse_time function also checks to make sure that nurse is being used.
        }*/
...
double ObjectiveNew(struct INSTANCE* ip, int report){
    ...
    int printType = 0; //Just for printing out statements in a particular form.
    ...
    for(int i = 0; i < ip->nNurses; ++i){ // For each index i in nurseOrder
        ...
        if(report > 0){
            if(printType == 0){
                printf("Nurse %d. Start time: %.2f, ", ni, (double)(ip->nurseWorkingTimes[ni][0]));
                printf("End time: %.2f, ", (double)(ip->nurseWorkingTimes[ni][1]));
                printf("Length of shift: %.2f\n", (double)(ip->nurseWorkingTimes[ni][2]));
            }
            else{
                int* timeNWTStart;
                timeNWTStart = MinsToTime((double) (ip->nurseWorkingTimes[ni][0]));
        ...
        for(int p = 0; p < ip->nJobs + 1; ++p){ //For each POSITION 0,...,nJobs + 1
            ...
            if(report > 0){
                ...
                if((p < ip->nJobs - 1) && (ip->allNurseRoutes[ni][p + 1] > -1)){
                    double travelTime = GetTravelTime(ip, ip->allNurseRoutes[ni][p], ip->allNurseRoutes[ni][p + 1]);
                    if(printType == 0){
                        printf("travel(TR): %.2f ", travelTime);
                    }
                    else{
                        int* timeTR;
                        timeTR = MinsToMinSecs(travelTime);
        ...
        if(report > 0){
            ...
            if(overtime < 0){
                double overtimePositive = overtime*-1;
                if(printType == 0){
                    printf("Overtime: -%.2f ", overtimePositive);
                }
                else{
                    int* timeOTPositive;
                    timeOTPositive = MinsToTime(overtimePositive);
    ...
    //if(ip->qualityMeasure == 6){ // Only for 'paper' quality.
    ...
    //} // END if(ip->qualityMeasure == 6) calculation for paper quality.
    ...
    /*if(ip->breakPoint % 100 == 0){
        printf("ip->breakpoint: %d, maxDiffWorkload: %.2f\n", ip->breakPoint, maxDiffWorkload);
    }
    ip->breakPoint += 1;*/
