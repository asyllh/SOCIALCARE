...
import literature_conversions as lc

# Display all rows and columns of dataframes in command prompt:
pd.set_option("display.max_rows", None, "display.max_columns", None)

def create_solve_inst(client_df, nurseshift_df, nurseday_df, planning_date, options_vector, wb_balance, quality_measure, max_time_seconds, random_seed):
...
def convert_dfs_inst(inst, client_df, nurseshift_df, nurseday_df, planning_date):
    ...
    for i in range(inst.nNurses):
        ...
        inst.nurseWorkingTimes[i][3] = 0 # NEW: 04/11/2021 Number of shifts
        inst.nurseWorkingTimes[i][4] = 0 # NEW: 04/11/2021 Durations of shifts only, not whole day.
    ...
    inst.totalsArray = np.zeros(23, dtype=np.float64) # NEW 04/06/2021
    ...
    # That is, col[4] = col[2] (total shift length) minus unavailable time (if the nurse has any). (04/11/2021)
    for i in range(inst.nNurses):
...
def create_solve_inst_aith(options_tuple, rSeed):
    
    # Unpack all the options:
    file_to_run, instance_type, quality_measure, ds_skill_type, max_time_seconds, verbose_level, load_from_disk, mkVNS, options_vector = options_tuple

    # i = prepare_instance(file_to_run, instance_type, quality_measure, ds_skill_type, max_time_seconds, verbose_level, load_from_disk, mkVNS, options_vector)
    ...
    inst.full_file_name = file_to_run
    inst.name = os.path.basename(file_to_run)
    inst.fname = os.path.basename(file_to_run)
    print('inst.full_file_name: ', inst.full_file_name, end='\n')
    print('inst.name: ', inst.name, end='\n')
    

    inst.quality_measure = quality_measure

    # print('Skill type was', i.DSSkillType)
    inst.DSSkillType = ds_skill_type
    # print('Skill type now is', ds_skill_type)
    # i.MAX_TIME_SECONDS = max_time_seconds
    # i.verbose = verbose_level
    inst.loadFromDisk = load_from_disk
    # print('max time seconds ' + str(i.MAX_TIME_SECONDS))
    print('Before calling, inst: ', inst)
    print('inst.DSSkillType ', inst.DSSkillType)
    # Generate the instance from the file:

    inst = lc.convert_aith_instance(file_to_run, inst)
    ...
    inst.timeMatrix = np.full((inst.nNurses, inst.nJobs), -1, dtype=np.float64) 
    inst.nurseWaitingTime = np.zeros(inst.nNurses, dtype=np.float64) 
    inst.nurseTravelTime = np.zeros(inst.nNurses, dtype=np.float64) 
    inst.violatedTW = np.zeros(inst.nJobs, dtype=np.float64) 
    inst.nurseWaitingMatrix = np.zeros((inst.nNurses, inst.nJobs), dtype=np.float64) 
    inst.nurseTravelMatrix = np.zeros((inst.nNurses, inst.nJobs), dtype=np.float64) 
    inst.totalsArray = np.zeros(23, dtype=np.float64) 
    ...
    # inst.init() #NOTE: NEED TO REMOVE!

    if (len(options_vector) == 100):
        inst.algorithmOptions = options_vector
    else:
        inst.algorithmOptions = default_options_vector()


    if quality_measure == 'ait_h':
        inst.algorithmOptions[0] = 0 # This is for 'ait_h', if 'paper' then algorithmOptions[0] = 6.
        inst.algorithmOptions[12] = 1
    elif quality_measure == 'paper':
        inst.algorithmOptions[0] = 6 # This is for 'paper', if 'ait_h' then algorithmOptions[0] = 0.
        inst.algorithmOptions[12] = 1
    
    inst.verbose = verbose_level 
    inst.MAX_TIME_SECONDS = max_time_seconds
    inst.solMatrix = np.zeros((inst.nNurses, inst.nJobs), dtype=np.int32)
    inst.exclude_nurse_travel = False

    saved_od_data = inst.od[0][0]
    print_all_call_data_python = options_vector[99] > 0
    inst.solve(randomSeed=rSeed, printAllCallData=print_all_call_data_python)
    inst.solve = []
    inst.lib = []
    inst.fun = []
    inst.Cquality = -1*inst.od[0][0]
    inst.od[0][0] = saved_od_data
    ...
    return inst
### --- End def create_solve_inst_aith --- ###
...
class INSTANCE(object):
    def __init__(self):
        ...
        # totalOvertime, maxOvertime, minSpareTime, maxSpareTime, shortestDay, longestDay, ait_quality, mk_quality, wb_quality, paper_quality, quality, mkMaxTardiness
    ...
    def instantiate_job_nurse_objects(self, client_df, nurseday_df):
        ...
        for j in range(self.nJobs):
            ...
            self.jobObjs[-1].ID = j # Set the ID for the object just added to jobObjs to be the index of the object (its position in the jobObjs list).

        self.nurseObjs = []
        for i in range(self.nNurses):
            self.nurseObjs.append(NURSE())
            self.nurseObjs[-1].ID = i # Set the ID for the object just added to nurseObjs to be the index of the object (its position in the nurseObjs list).
    ...
    def instantiate_job_nurse_objects_aith(self):
        # NOTE NEW: 10/11/2021 - this is only for initialising job and nurse objects when the instance_type (problem instance file) is 'ait_h'.
        self.jobObjs = []
        for j in range(self.nJobs):
            self.jobObjs.append(JOB())
            self.jobObjs[-1].ID = j
        ...
        for i in range(self.nNurses):
            self.nurseObjs.append(NURSE())
            self.nurseObjs[-1].ID = i
    ### --- End def instantiate_job_nurse_objects_aith --- ###
    ...
    def solve(self, randomSeed=0, printAllCallData=False):
        ...
        print('\nEnd of Python program - calling C program (GRASP-VNS-ALL)')
        ...
        # print('totals array:')
        # for i in range(len(self.totalsArray)):
        #     print(self.totalsArray[i])
        # print('violated TW:')
        # for i in range(len(self.violatedTW)):
        #     print(self.violatedTW[i])
        # exit(-1)
    ...
    def full_solution_report(self, report=2, doPlots=True):
        ...
        # self.maxTardiness = self.totalsArray[7]
...
def default_options_vector():
    ...
    ov[58] = -5 # alpha_8 Max Travel Time NEW: 06/11/2021
    ov[59] = -5 # alpha_9 Max Waiting Time NEW: 06/11/2021
    ov[60] = -2000 # alpha_10 NEW FOR NEW WORKLOAD BALANCE VARIABLE 06/11/2021
