...
def worker_task_new(idict, options_vector, random_seed):
    ...
    # inst = prepare_instance(file_to_run, instance_type, quality_measure, ds_skill_type, max_time_seconds, verbose_level, load_from_disk, mkVNS, options_vector)
...
def worker_task(options_tuple, rSeed):
    ...
    i.lib = []
    i.fun = []
    i.Cquality = -1*i.od[0][0] # Cquality = solution quality from C program, it's stored in od[0][0] (shortcut, should probably change this!)
    i.od[0][0] = saved_od_data
    # quality = i.full_solution_report(doPlots=False, report=0)
    # print('Another quality ' + str(quality)) # print('i.od[0][0]  ' + str(i.od[0][0]))
    return i
### --- End def worker_task --- ###

def prepare_instance(file_to_run, instance_type, quality_measure='default', ds_skill_type='strictly-shared', max_time_seconds=-1, verbose_level=-1, load_from_disk=False, mkVNS=False, options_vector=[]):
...
class INSTANCE(object):
    ...
    def unpickle_instance(self, name):
        ...
        instance_obj.init() # NOTE: CHECK THIS .init()
        # self.init()
        # print('Done.')
        return instance_obj
    ...
    def objs_info_to_c_format(self):
        ...
        self.jobTimeInfo = np.zeros((self.nJobs, 3), dtype=np.int32)
        self.dependsOn = np.zeros(self.nJobs, dtype=np.int32) - 1
        self.mk_maxd = np.zeros(self.nJobs, dtype=np.int32)
        ...
        if loadFromDisk:
            ...
            nursesGo = np.array(nursesGo) # Nurses going from depot (nurse_travel_from_depot)
            nursesReturn = np.array(nursesReturn) # Nurses returning to depot (nurse_travel_to_depot)
        ...
        self.capabilityOfDoubleServices = np.asarray(self.capabilityOfDoubleServices, dtype=np.int32)
        self.dependsOn = np.asarray(self.dependsOn, dtype=np.int32)
