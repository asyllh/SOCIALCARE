...
MODULE_API int python_entry(int nJobs_data, int nNurses_data, int nSkills_data, int verbose_data, float MAX_TIME_SECONDS, double* od_data, double* nurse_travel_from_depot_data,
	double* nurse_travel_to_depot_data, int* nurseWorkingTimes_data, int* jobTimeInfo_data, int* jobRequirements_data, int* nurseSkills_data, int* solMatrixPointer, int* doubleService_data,
	...
	struct INSTANCE inst = instance_from_python(nJobs_data, nNurses_data, nSkills_data, verbose_data,MAX_TIME_SECONDS, od_data, nurse_travel_from_depot_data, nurse_travel_to_depot_data,
		nurseWorkingTimes_data, jobTimeInfo_data, jobRequirements_data, nurseSkills_data, doubleService_data, dependsOn_data, mk_mind_data, mk_maxd_data, capabilityOfDoubleServices,
...
// int report_solution(int startTime, int nJobs, int nNurses, int**od, int**solMatrix, int** jobTimeInfo, int* jobDuration, struct INSTANCE * ip)
...
// int best_exchange(struct INSTANCE * ip)
// {
// 	int * nurseRouteJ = malloc(ip->nJobs * sizeof(int));

// 	for (int ni = 0; ni < ip->nNurses; ++ni)
// 	{
// 		get_nurse_route(ip, ni, nurseRouteJ);
...
// 			if (ni == nj)
// 				continue
// 			get_nurse_route(ip, nj, nurseRouteJ);
// 		}
// 	}
	...
	for (int i = 0; i < ip->nJobs; ++i) { //For each job i=0,...,nJobs
		...
		for (int tn = 0; tn < ip->nNurses; ++tn) { //For each nurse tn=0,...,nNurses
			...
			if (ip->doubleService[jobA]) { //If jobA is a double service
				if (check_skills_ds_first(ip, jobA, tn) < 1) { //If no other nurse can do jobA with nurse tn. (capabilityOfDoubleService, if there's another nurse that can do the job with tn then =1, else = 0)
					if (DEBUG_PRINT > 0) {
		...
		for (int job_from_ni = 0; job_from_ni < ip->nJobs; ++job_from_ni) { //For each job_from_ni 0,...,nJobs
			...
			for (int nj = ni + 1; nj < ip->nNurses; ++nj) { //For each nurse nj = ni+1,..., nNurses
				...
				if (ip->doubleService[job_from_ni]) { //If job_from_ni is a double service
					if (check_skills_ds(ip, job_from_ni, secondNurseI, nj) < 1) { // returns ip->capabilityOfDoubleServices[secondNurseI][nj][jobdsindex], <1 means nj is not skilled to do the job with secondNurseI
						continue;
					}
				}
