...
%	\For{$j \in r_i$}
	\ForEach{job $j$ in nurse $i$'s route $r_i$}
		\State $a(i,j) \gets a(i, j-1) + w_{ij-1} + s_{j-1} + T_i(j-1, j)$
		\State $i' \gets \Null$, $j' \gets \Null$, DJ $\gets \False$, DS $\gets \False$ %, secondN $\gets \False$
		\If{$j \in J'$} % D : J' -> DJ, returns other job in pair with job j, NJ : J -> N, returns the nurse assigned to the job j
...
%				\State $[e_j^{(j')}, l_j^{(j')}] \gets$ dependent time window, based on $t_{j'}$ set by $i'$
			\EndIf
		\EndIf %if j in J'
		% 1. if nurse only has one shift
		\If{nurse $i$ only has one shift, $g_i = 1$}
		...
		% 2. Else nurse has multiple shifts, arrival is between shifts
		...
		% 3. Else arrival is in valid shift but job extends into break
		\ElsIf{$\max\{a(i,j), e_j\} + s_j > v(\pi_{f}^{i}) \AAnd f \neq g_i$}
			\State $t_j, w_{ij}, z_j, \tilde{z}_j, f \gets$ \textsc{FindValidTime}($i, j, f, a(i,j), i', j'$, DS, DJ)
		% 4. Job can be performed feasibly, or it is the final shift of nursei 
		\Else
			\If{DJ $= \True \Or$DS $= \True$}			
				\State $w_{ij} \gets \max\{0, ((t_{j'} + e_j^{(j')}) - a(i,j))\}$
				\State $t_j \gets a(i,j) + w_{ij}$
...
%		\State $[e_j^{(j')}, l_j^{(j')}] \gets$ dependent time window, based on $t_{j'}$ set by $i'$
	...
	\While{$\Not$ feasible}
		...
		% A. nurse i is the second nurse being assessed, i.e. job is DJ or DS.
%		\If{secondN $= \True$}
		\If{DJ $= \True \Or$DS $= \True$}
			% 1a. currentTime is LATER than latest allowed start time
%			\If{$\tilde{a}(i,j) > l_j^{(j')}$}
...
%					\State $f \gets \tilde{f}$
%				\EndIf
			% 2a. job cannot be performed feasibly in shift, extends into break
%			\ElsIf{$\max\{\tilde{a}(i,j), e_j^{(j')}\} + s_j > v(\pi_{\tilde{f}}^{i}) \AAnd \tilde{f} \neq g_i$}
%			\If{$\max\{\tilde{a}(i,j), e_j^{(j')}\} + s_j > v(\pi_{\tilde{f}}^{i}) \AAnd \tilde{f} \neq g_i$} %\hspace{0.2mm}\textbf{continue}
			\If{$\max\{\tilde{a}(i,j), (t_{j'} + e_j^{(j')})\} + s_j > v(\pi_{\tilde{f}}^{i}) \AAnd \tilde{f} \neq g_i$} %\hspace{0.2mm}\textbf{continue}
				\Continue
			% 3a. nurse can perform job feasibly in shift OR shift \tilde{f} is the last shift of the day
			\Else
				\State feasible $\gets \True$
				\State $t_j \gets \tilde{a}(i,j)$
				\State $w_{ij} \gets \max\{0, (t_j - a(i,j))\}$
				...
				\State $f \gets \tilde{f}$
			\EndIf
		% End A. If secondN = True
		% B. nurse i is the first nurse assessing DJ/DS OR job is not a DJ/DS
		\Else
			% 1b. job cannot be performed feasibly in shift, extends into break
			\If{$\tilde{a}(i,j) + s_j > v(\pi_{\tilde{f}}^{i}) \AAnd \tilde{f} \neq g_i$} %\hspace{0.2mm}\textbf{continue}
				\Continue
			% 2b. nurse can perform job feasibly in shift OR \tilde{f} is the last shift of the day
			\Else
				\State feasible $\gets \True$
				\State $t_j \gets \tilde{a}(i,j)$
				\State $w_{ij} \gets \max\{0, (t_j - a(i,j))\}$
...
\If{$(j, j') \in DS$}
\State DS $\gets \True$
\If{secondN $= \True \AAnd a(i,j) > t_{j'}$} %can also use l_j^(j'), latest ST allowed for j based on j'
\State feasible $\gets \False$
\Break
\EndIf % arrival later than latest start time
\Else 
...
%Need to find out if this job j is before or after j' in the schedule
\State $[t_{j'} + e_j^{(j')}, t_{j'} + l_j^{(j')}] \gets$ dependent time window, based on $t_{j'}$ set by $i'$
\If{$a(i,j) > t_{j'} + l_j^{(j')} \AAnd$gap $= \False$} % current time is later than latest allowed start based on start time of j' and no gap tard allowed
\State feasible $\gets \False$
\Break
\ElsIf{$a(i,j) > t_{j'} + l_{j}^{(j')} \AAnd$gap $= \True$}
...
\EndIf %(j, j') in DS
\EndIf %if j in J'
\If{$a(i,j) > v(\pi_{f}^{i}) \AAnd a(i,j) < u(\pi_{f+1}^{i}) \AAnd f \neq g_i$}
\State $v \gets v(\pi_{f}^{i})$
...
\State $a(i,j) \gets a(i, j-1) + w_{ij-1} + s_{j-1} + T_i(j-1, j)$
\State $i' \gets \Null$, $j' \gets \Null$
\State DJ $\gets \False$, DS $\gets \False$, secondN $\gets \False$
\If{$j \in J'$}
\State $j' \gets D(j)$ % D : J' -> DJ, returns other job in pair with job j
\State $i' \gets NJ(j')$ % NJ : J -> N, returns the nurse assigned to the job j
\If{$i$ appears later than $i'$ in $\mathcal{O}$}
...
\ElsIf{$a(i,j) > v(\pi_{f}^{i}) \AAnd a(i,j) < u(\pi_{f+1}^{i}) \AAnd f \neq g_i$}
\State $v \gets v(\pi_{f}^{i})$
\State $t_j, w_{ij}, z_j, \tilde{z}_j, f \gets$ \textsc{FindValidTime}($i, j, f, v, i', j'$, DS, DJ, secondN)
% 3. Else arrival is in valid shift but job extends into break
\ElsIf{$\max\{a(i,j), e_j\} + s_j > v(\pi_{f}^{i}) \AAnd f \neq g_i$}
\State $t_j, w_{ij}, z_j, \tilde{z}_j, f \gets$ \textsc{FindValidTime}($i, j, f, a(i,j), i', j'$, DS, DJ, secondN)
% 4. Job can be performed feasibly, or it is the final shift of nursei 
\Else
\State $w_{ij} \gets \max\{0, (e_j \chng{e_j^{(j')}} - a(i,j))\}$
\State $t_j \gets a(i,j) + w_{ij}$
