...
The arrival time $a(i,j)$ of nurse $i$ at job $j \in r_i$ is calculated as follows \chng{This is not true with shifts}:
\begin{equation}
    a(i,j) = st(i) + \sum_{h=0}^{j' - 1} w_{ik^i_h} + s_{k^i_h} + T_i(k^i_h, k^i_{h + 1}), \label{eq:arrivaltime}
\end{equation}

\noindent where $j'$ denotes the position of job $j$ in the route $r_i$, i.e. $j = k^i_{j'} \in r_i$. Note that there are no waiting or service times at the depot; that is, $w_{ik^i_0} = 0$ and $s_{k^i_0} = 0 $ for all $i \in N$. Furthermore, let $en_{\text{act}}(i)$ be the actual end time of a nurse $i \in N$, that is: \chng{Is this different from $a(i, k_{0}^i)$?}
\begin{equation}
...
\noindent The \textsc{CalculateJobTimes} function in \ref{alg:shiftwork} describes how start times, waiting times and tardiness are calculated for each nurse $i \in N$. For each job $j \in r_i$, the \chng{arrival time is evaluated using the arrival time, waiting time, service time and travel time of the previous job. - key, but not written in (7)} Line 1 covers the case where nurse $i$'s day comprises just one shift, i.e. $g_i = 1$. Line 9 is for the case where the duration of the current job $j$ extends in to a break period, whilst Line 14 considers the event that the arrival time of nurse $i$ and job $j$ would be within a break \chng{what about case where there are shifts but job is okay to do?}. In both of these cases, the function \textsc{FindValidTime} is used. This function scans through the nurse's day to find the next available time period in which the nurse can feasibly perform the job $j$, and returns the start time of this period, $t_j$. If the job $j$ is a DS, then \textsc{FindValidTime} seeks a start time that is suitable for the current nurse $i$ and the second nurse on the job.
...
%			\State $f \gets$ shift containing time $t_j$
		\ElsIf{$g_i \neq 1 \AAnd a(i,j) > v(\pi_{f}^{i}) \AAnd a(i,j) < u(\pi_{f+1}^{i})$}
...
\begin{algorithmic}[1]
	...
	\If{$j$ is a double service}
		...
		\If{$i > \tilde{i}$}
			...
			\If{$\tilde{a}(i,j) > t_j$} %t_j is the start time for j, already set for 1st nurse earlier
				% infeasible, exit
				\State valid $\gets \False$
				\Break
			\EndIf
	...
	\While{$\Not$ valid}
		...
		\If{DS$= \True$}
			\If{second $= \False$}
				...
				\EndIf
			\ElsIf{second $= \True$}
				\If{$\tilde{a}(i,j) > t_j$} %t_j is the start time of the job from 1st nurse, already set
					%infeasible, exit
					\State valid $\gets \False$
					\Break
				\ElsIf{$t_j + s_j > v(\pi_{\tilde{f}}^{i}) \AAnd \tilde{f} \neq g_i$}
					\Continue
				\Else
					\State valid $\gets \True$
					%\State $t_j \gets \tilde{a}(i,j)$
